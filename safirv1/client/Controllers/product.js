...
Template.add_review.events({
    'click #commentok': function(e, tpl) {
        ...
        if (TAPi18n.getLanguage() == 'fa') {
            Bert.alert('اضافه کردن نظر موفقیت است', 'success', 'growl-bottom-right');
        } else {
            Bert.alert('Add Review is success', 'success', 'growl-bottom-right');
        }
        $('.close').click();
    }
});
